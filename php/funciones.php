<?php
	...
	function subir_imagen($tipo,$imagen,$email) {
		...
		if(strstr($tipo,"image")) {
			//saber que tipo de extencion es la img
			if(strstr($tipo,"jpeg"))
				$extension = ".jpg";
			...
			//Si la img es mayor en su ancho que 420px, reajusto el tamaño
			if($ancho_img > $ancho_img_deseado) {
				//reajustar, por una regla de 3 obtengo el alto de la imagen de manera proporcional al ancho nuevo que sera de 420

				$nuevo_ancho_img = $ancho_img_deseado;
				$nuevo_alto_img = ($alto_img/$ancho_img)*($nuevo_ancho_img);

				//Creo una imagen en color real con las nuevas dimenciones
				...
				//Se sube la foto
				move_uploaded_file($imagen, $destino) or die("No se pudo subir la imagen XD");

				//Ejecutar la funcion para borrar posibles imagenes dobles para el perfil
				$nombre_img = "../img/fotos/".$email;
				borrar_imagenes($nombre_img, $extension);
			}
			//Asignar el nombre de la foto que se guardara en la DB como cadena de texto
